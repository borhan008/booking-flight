...
export default function BookASeat() {
  ...
  useEffect(() => {
    const fetchFlight = async() => {
      const res = await fetch(`https://flight-server-six.vercel.app/api/flights/${id}`);
  ...
  const handleBookNow = async() => {
    ...
    const seatBook = await fetch(`https://flight-server-six.vercel.app/api/bookings`, {
      method : "POST",
      headers : {
    ...
    if(seatBookData.ok){
      ...
      toast.error("Something went wrong while booking the seat.");
    }
  }

  const handleConfirmSeat = async() => {
    const seatBook = await fetch(`https://flight-server-six.vercel.app/api/bookings/confirm`, {
      method : "POST",
      headers : {
       "Content-Type": "application/json",
       "Authorization": `Bearer ${localStorage.getItem("token")}`
  ...
  return (
    ...
    {countDown ?
        ...
        <section className='max-w-xl mx-auto py-16  '>
           ...
           {counting && <h2 className='text-black/80 text-2xl text-center mb-4'>Please wait {counting}s to confirm</h2>
}
            {counting == 0 && <button  className='border border-blue-800 text-center- w-[80%] mx-auto text-center p-2 rounded-full block text-blue-800 hover:bg-blue-800 hover:text-white transition' onClick={handleConfirmSeat}>Confirm Seats</button>}
            </div>
         </section>
   :
      <section className='max-w-7xl mx-auto py-16'>
        ...
        {flight &&     <div className="bg-white rounded-xl shadow-md max-w-lg w-full overflow-hidden pb-4">
                ...
                <div className="bg-blue-800 text-white px-6 py-4 flex items-center justify-between">
                    ...
                    <span> {Math.max(1, seatBookingsIds.length)*flight.price}</span>
                    </div>
                    </div>
      ...
      <div key={singleFlight.seatNumber} className='flex-1 min-h-10 min-w-10 '>
      {singleFlight.isBooked ? <div  className={`w-full  min-h-10 transition rounded-full flex justify-center items-center text-black border border-black text-black opacity-20`}>
      {singleFlight.seatNumber}
      </div> : <div  className={`w-full  min-h-10 transition rounded-full flex justify-center items-center text-black border border-black text-black hover:bg-blue-800 hover:text-white ${seatBookingsIds.includes(singleFlight._id) ? 'bg-blue-800 text-white' : ''}`} onClick={() => handleAddSeat(singleFlight._id)}>
      {singleFlight.seatNumber}
      </div>}</div>
                ...
                <button  className='border border-blue-800 text-center- w-[80%] mx-auto text-center p-2 rounded-full block text-blue-800 hover:bg-blue-800 hover:text-white transition' onClick={handleBookNow}>Book Now</button>
            </div>}
